...
exports.obsMgr = {
  ...
  getAllObsAndNameOrg : function(cb){ //sort by organisaition type
    mysqlMgr.connect(function (conn) {
      conn.query('SELECT * FROM  `observers` obs, `organisaition` org WHERE org.`status` =1 AND obs.`status` =1 AND obs.`registration_org` = org.`registration_no`', function(err, result) {
        conn.release();
        if(err) {
          util.log(err);
        } else {
