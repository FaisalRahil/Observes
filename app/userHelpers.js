...
module.exports = {
  ...
  isRoot : function (req,res,next) {
    if (req.isAuthenticated() && req.session.id_office<0) { return next(); }
    res.redirect('/')
  },

  isLogin : function (req,res,next) {
    if (!req.isAuthenticated()) { return next(); }
    if (req.isAuthenticated() && req.session.id_office<0 ){res.redirect('/admin/org'+ req.session.office_idoffice);}
    if (req.isAuthenticated() && req.session.id_office>0 ){res.redirect('/manager/org');}
  },
