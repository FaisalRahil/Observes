$(document).ready(function () {
  ...
  $("#table").bootstrapTable({
    ...
    columns: [
      ...
      },
      {
        ...
        // checkbox:true,
        title:
          '<button id="print" class="btn btn-success btn-sm"><span class="glyphicon glyphicon-plus"></span><span class="text-none"> الطباعة</span></button>',
  ...
  function operateFormatter(value, row, index) {
    return [
      '<a id="viewMedia" class="btn btn-xs btn-primary" href="/admin/editGuestObs/' +
        value +
  ...
  function operateFormatter1(value, row, index) {
    return [
      '<button id="deleteGuest" data-toggle="modal" href="#deleteGuestObsModule" class="btn btn-xs btn-danger" value="' +
        value +
  ...
  function status(value, row, index) {
    if (value == 1) {
      return ['<i class="glyphicon glyphicon-ok"></i>'].join("");
    } else {
      return [""].join("");
    }
  }
  function nationality(value, row, index) {
    //console.log($.nat);
    return [$.nat[value - 1].text.name].join("");
  }
